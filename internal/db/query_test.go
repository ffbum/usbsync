...
func TestListEntriesReturnsDeletionFlagsAndRevision(t *testing.T) {
	...
	if _, err := store.DB.Exec(`
		...
		VALUES
			('docs/a.txt', 'docs/a.txt', 'file', 5, 10, 0, 3, 'm1', '2026-03-30T00:00:00Z'),
			('docs/b.txt', 'docs/b.txt', 'file', 0, 0, 1, 4, 'm1', '2026-03-30T00:00:00Z')
	`); err != nil {
...
func TestListEntriesAtRevisionReturnsSnapshotForThatPointInTime(t *testing.T) {
	...
	if _, err := store.DB.Exec(`
		INSERT INTO change_log(revision, machine_id, op, path_key, display_path, kind, base_revision, size, content_md5, blob_id, created_at)
		VALUES
			(1, 'm1', 'mkdir', 'docs', 'docs', 'dir', 0, 0, '', '', '2026-03-30T00:00:00Z'),
			(2, 'm1', 'add', 'docs/a.txt', 'docs/a.txt', 'file', 0, 5, 'hash-a-v1', 'hash-a-v1', '2026-03-30T00:00:00Z'),
			(3, 'm2', 'modify', 'docs/a.txt', 'docs/a.txt', 'file', 2, 5, 'hash-a-v2', 'hash-a-v2', '2026-03-30T00:01:00Z'),
			(4, 'm2', 'delete', 'docs/a.txt', 'docs/a.txt', 'file', 3, NULL, '', '', '2026-03-30T00:02:00Z')
	`); err != nil {
...
func TestListChangesAfterJoinsDisplayName(t *testing.T) {
	...
	if _, err := store.DB.Exec(`
		...
		VALUES
			(1, 'm1', 'add', 'docs/a.txt', 'docs/a.txt', 'file', 0, '2026-03-30T00:00:00Z'),
			(2, 'm1', 'modify', 'docs/a.txt', 'docs/a.txt', 'file', 1, '2026-03-30T00:01:00Z')
	`); err != nil {
		t.Fatalf("seed changes: %v", err)
	}
